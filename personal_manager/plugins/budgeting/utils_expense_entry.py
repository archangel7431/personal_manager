...
def checking_for_directory(directory_path: Path) -> None:
    """
...
def checking_for_file(filepath: Path) -> None:
    """
...
def append_to_csv(filepath: Path, data: list[tuple[date, str, str]])-> None:
    """
...
def expense_entry(write_file_path: str = None) -> None:
    """
    Main function to run the program.
    Args:
        write_file_path (str): The path to the CSV file where data will be written. 
        If None, defaults to "data/budget.csv" in the root directory.
    Returns:
        None
    """
    ...
    if write_file_path is None:
        ...
        write_file_path = Path.cwd() / "data" / "budget.csv"
    else:
    ...
    directory_path = write_file_path.parent
...
if __name__ == "__main__":
    ...
    # Use root 'data/' directory by default
    expense_entry()
